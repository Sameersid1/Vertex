...
interface OpenFile extends TemplateFile{
    id:string;           //taki check kr paye if file already opened
...
export const useFileExplorer=create<FileExplorerState>((set,get)=>({
    ...
    setPlaygroundId(id){
        set({playgroundId : id});   //same id lelo playground wali
    },
    ...
    openFile:(file)=>{
        ...
        const {openFiles}=get();
        const existingFile=openFiles.find((f)=>f.id===fileId)   //jo already khuli khol de
    ...
    const { templateData } = get();    //--Entire structure
    if (!templateData) return;

    try {
      const updatedTemplateData = JSON.parse(JSON.stringify(templateData)) as TemplateFolder;
      const pathParts = parentPath.split("/");
      let currentFolder = updatedTemplateData;  //initially whole root.updating makes it part root of any folder
  ...
  handleRenameFile: async (
    ...
    const oldFileId = generateFileId(file, templateData);         //src/components/Button.tsx----id->abc13
    const newFile = {
      ...file,
